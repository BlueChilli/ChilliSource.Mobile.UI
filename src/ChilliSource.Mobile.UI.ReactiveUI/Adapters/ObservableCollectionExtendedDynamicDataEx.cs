...
namespace ChilliSource.Mobile.UI.ReactiveUI
{
	...
	public static class ObservableCollectionExtendedDynamicDataEx
	{
		/// <summary>
		...
		public static IObservable<IChangeSet<T>> Bind<T>([NotNull] this IObservable<IChangeSet<T>> source,
			[NotNull] IObservableCollectionExtended<T> targetCollection, int resetThreshold = 25)
		{
			if (source == null) throw new ArgumentNullException(nameof(source));
			if (targetCollection == null) throw new ArgumentNullException(nameof(targetCollection));

			var adaptor = new ObservableListToObservableCollectionExtendedAdapter<T>(targetCollection, resetThreshold);
